...
poverty_data <- data.frame(
  ...
  poverty_rate = c(55, 10, 35, 30, 18, 25, 5, 3, 2, 3),  
  # Persentase populasi dunia 
  population_share = c(18, 20, 4, 3, 3, 3, 2, 2, 5, 2)  
)
```

```{r fig.height=5, fig.width=10, dpi=300}
# Bar chart pertama: Persentase orang miskin
p1 <- ggplot(poverty_data, 
             aes(x = poverty_rate, y = reorder(country, poverty_rate))) +
...
p2 <- ggplot(poverty_data, 
             aes(x = population_share, y = reorder(country, population_share))) +
  geom_bar(stat = "identity", fill = "orange", width = 0.5) +
...
ggplot(poverty_mari) +
  geom_rect(aes(xmin = xmin, xmax = xmax, 
                ymin = 0, ymax = poverty_rate, 
                fill = color_group)) +
  geom_text(aes(x = (xmin + xmax) / 2, 
                y = poverty_rate / 2, 
                label = country), size = 3.5) +
  scale_x_continuous(labels = percent_format(scale = 1), 
                     breaks = seq(0, 60, 5), expand = c(0, 0)) +
...
```{r fig.height=5, fig.width=10, dpi=300}
# Scatterplot
ggplot(poverty_data, aes(x = population_share, y = poverty_rate)) +
  geom_point(size = 4, color = "#FF6600") +  # Warna oranye sesuai gambar
  geom_text(aes(label = country), hjust = -0.2, vjust = -0.5, size = 4) +  # Label negara
  scale_x_continuous(labels = scales::percent_format(scale = 1), limits = c(0, 35)) +  
...
poverty_long <- poverty_data %>%
  # Mengasumsikan total populasi 1000 untuk visualisasi
  mutate(
    total_pop = round(population_share * 1000),
    poor_pop = round(total_pop * poverty_rate / 100),
    non_poor_pop = total_pop - poor_pop
  ) %>%
  select(country, poor_pop, non_poor_pop) %>%
  tidyr::pivot_longer(
    cols = c(poor_pop, non_poor_pop),
    names_to = "status",
    values_to = "count"
  ) %>%
  # Mengulang setiap baris sesuai dengan count
  uncount(count) %>% 
...
```{r fig.height=5, fig.width=10, dpi=300}
# Membuat mosaic plot
ggplot(data = poverty_long) +
  geom_mosaic(aes(x = product(country), fill = status)) +
  scale_fill_manual(
    values = c("poor_pop" = "#FF9999", "non_poor_pop" = "#99CC99"),
    labels = c("Miskin", "Tidak Miskin"),
    name = "Status"
  ) +
  labs(
    title = "Distribusi Kemiskinan di Berbagai Negara",
    x = "Negara",
    y = "Proporsi"
  ) +
  theme_minimal() +
