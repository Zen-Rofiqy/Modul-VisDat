...
poverty_data <- data.frame(
  country = c("India", "China", "Nigeria", "Bangladesh", "Indonesia",
              "Pakistan", "Brazil", "Mexico", "United States", "Russia"),
  # Persentase total populasi dunia
  population_share = c(18, 17, 4, 3, 3, 3, 2, 2, 5, 2),  
  # Persentase penduduk miskin
  poverty_rate = c(55, 15, 30, 25, 20, 22, 10, 5, 3, 5)  
)

# Urutkan dan hitung posisi, tapi reverse urutan x
poverty_data <- poverty_data %>%
  arrange(desc(poverty_rate)) %>%
  mutate(
    xmin = max(cumsum(population_share)) - cumsum(population_share),
    xmax = max(cumsum(population_share)) - c(0, head(cumsum(population_share), -1)),
    color_group = factor(country, levels = country) # Faktor untuk warna
  )
...
ggplot(poverty_data) +
  ...
  scale_x_continuous(labels = percent_format(scale = 1), breaks = seq(0, 60, 5), expand = c(0, 0)) +
