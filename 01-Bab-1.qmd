...
# Buat dataset
df <- data.frame(
  Country = c("India", "China", "Nigeria", "Bangladesh", "Indonesia",
              "Pakistan", "Brazil", "Mexico", "United States", "Russia"),
  PopulationShare = c(18, 17, 4, 3, 3, 3, 2, 2, 5, 2),  # Persentase dari total populasi dunia
  PovertyRate = c(55, 15, 30, 25, 20, 22, 10, 5, 3, 5)  # Persentase penduduk miskin
)

# Konversi ke proporsi untuk Marimekko Chart
df <- df %>%
  mutate(PopWidth = PopulationShare / sum(PopulationShare) * 100,  # Lebar batang
         PovHeight = PovertyRate,
         xmax = cumsum(PopWidth),
         xmin = lag(xmax, default = 0))  # Menentukan batas batang
```

```{r fig.height=5, fig.width=10, dpi=300}
# Bar chart pertama: Persentase orang miskin
p1 <- ggplot(df, aes(x = PovertyRate, y = reorder(Country, PovertyRate))) +
  geom_bar(stat = "identity", fill = "orange") +
  labs(title = "Percent of people with less than $5.20/day",
       x = "Percent of people with less than $5.20/day", y = "") +
  theme_minimal()

# Bar chart kedua: Persentase populasi dunia
p2 <- ggplot(df, aes(x = PopulationShare, y = reorder(Country, PopulationShare))) +
  geom_bar(stat = "identity", fill = "orange") +
  labs(title = "Population (as percent of all countries)",
       x = "Population Share", y = "") +
  theme_minimal()
...
poverty_data <- data.frame(
  country = c("India", "China", "Nigeria", "Bangladesh", "Indonesia",
              "Pakistan", "Brazil", "Mexico", "United States", "Russia"),
  # Persentase dari total populasi dunia
  population_share = c(18, 17, 4, 3, 3, 3, 2, 2, 5, 2),  
  # Persentase penduduk miskin
  poverty_rate = c(55, 15, 30, 25, 20, 22, 10, 5, 3, 5)  
)

# Mengurutkan data berdasarkan population_share
poverty_data <- poverty_data[order(poverty_data$population_share, decreasing = TRUE), ]

# Menghitung posisi x untuk setiap bar
poverty_data$xmin <- c(0, head(cumsum(poverty_data$population_share), -1))
poverty_data$xmax <- cumsum(poverty_data$population_share)
...
ggplot(poverty_data) +
  ...
  scale_y_continuous(labels = scales::percent_format(scale = 1),
                    limits = c(0, 60),
                    expand = c(0, 0)) +
  labs(title = "High population, high poverty",
       x = "Population as share of total",
       y = "Percent of people with less than $5.20/day") +
  theme_minimal() +
  theme(
...
poverty_data <- data.frame(
  country = c("India", "China", "Nigeria", "Bangladesh", "Indonesia",
              "Pakistan", "Brazil", "Mexico", "United States", "Russia"),
  population_share = c(18, 17, 4, 3, 3, 3, 2, 2, 5, 2),  # Persentase total populasi dunia
  poverty_rate = c(55, 15, 30, 25, 20, 22, 10, 5, 3, 5)  # Persentase penduduk miskin
)

# Urutkan berdasarkan poverty_rate agar lebih mirip contoh
poverty_data <- poverty_data %>%
  arrange(desc(poverty_rate)) %>%
  mutate(
    xmin = c(0, head(cumsum(population_share), -1)),
    xmax = cumsum(population_share),
    color_group = factor(country, levels = rev(country)) # Faktor untuk warna berbeda
  )

# Warna khusus untuk tiap negara
color_palette <- c("India" = "#FFD27D", "China" = "#FFC34A", "Nigeria" = "#FFA500",
                   "Bangladesh" = "#FF8C00", "Indonesia" = "#FF7F50", "Pakistan" = "#FF6B00",
                   "Brazil" = "#FF4500", "Mexico" = "#E63900", "United States" = "#CC3300", "Russia" = "#B22222")

# Marimekko Chart
ggplot(poverty_data) +
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = 0, ymax = poverty_rate, fill = color_group)) +
  geom_text(aes(x = (xmin + xmax) / 2, y = poverty_rate / 2, label = country), size = 3.5) +
  scale_x_continuous(labels = percent_format(scale = 1), breaks = seq(0, 20, 5), expand = c(0, 0)) +
...
```{r fig.height=5, fig.width=10, dpi=300}
# Separate bar charts
# 1. Poverty Rate Chart
poverty_bar <- ggplot(poverty_data, aes(x = reorder(country, poverty_rate), y = poverty_rate)) +
...
# 2. Population Share Chart
population_bar <- ggplot(poverty_data, aes(x = reorder(country, population_share), y = population_share)) +
  geom_col(fill = "#FF6B6B") +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  theme_minimal() +
  labs(title = "Population (as percent of all countries)",
