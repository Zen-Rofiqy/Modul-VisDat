...
```{r fig.height=5, fig.width=10, dpi=300}
# Menambahkan kategori tingkat kemiskinan
poverty_data <- poverty_data %>%
  mutate(
    poverty_level = case_when(
      poverty_rate >= 50 ~ "Less than $5.50/day",
      poverty_rate >= 20 ~ "Less than $3.20/day",
      TRUE ~ "Less than $1.90/day"
    )
  )

# Mosaic Chart
ggplot(poverty_data) +
  geom_mosaic(aes(x = product(country, population_share), 
                  fill = poverty_level)) +
  scale_fill_manual(values = c("Less than $5.50/day" = "#F4A000",
                               "Less than $3.20/day" = "#E1573C",
                               "Less than $1.90/day" = "#377EB8")) +
  labs(title = "High population, high poverty",
       x = "Population as share of total",
       y = "Country",
       fill = "Poverty Level") +
